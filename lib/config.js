...
const cleanConfig = async (config) => {
  try {
    ...
    if (cleanServerPath) {
      config.cleanServerPath = cleanServerPath === true || cleanServerPath === 'true';

      if (config.cleanServerPath) {
        if (helper.validate.isRoot(serverPath)) {
          throw new Error('Due to security reasons cleanServerPath can\'t be used while serverPath is pointing at server root');
        }

        const randomDirectory = `../sd-${Date.now()}${Math.floor(Math.random() * (9999 - 1000 + 1) + 1000)}`;
        config.tempServerPath = path.join(serverPath, randomDirectory);
      }
    }
